...
- **Service Deployment Policy**: The agent is permitted to run long-running service CLIs (like web or API servers) in the background to verify development, but MUST explicitly terminate those processes immediately after testing is complete. Orphaned service CLI processes are strictly forbidden.
...
- **Tailwind**: Default to HTML + Tailwind unless another stack is explicitly requested.
